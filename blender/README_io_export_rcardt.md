...
パネル下部に**四角形以外の面の枚数**が常時表示され、
**「Select Non-Quad Faces」** ボタンで該当面を選択できます（§6参照）。
...
- **Non-Quad Faces**: 四角形以外の面があった場合の挙動（下記）
...
ゲームは全サーフェスに対して**四角形の描画コマンド**を発行するため、
三角形は描画が破綻し、5角形以上はそもそもフォーマットに収まりません
（市販版15車種3,341面はすべて四角形）。

**エクスポート時に四角形以外の面があると、既定で中断します。**
どのオブジェクトに三角形/N-gonが何枚あるかを表示し、ファイルは書き出しません。

```
The game only draws quads correctly, and these faces are not quads:
'Body': 3 triangle(s), 2 n-gon(s). Use 'Select Non-Quad Faces' in the
RCARDT Node Editor to find them, ...
```

### 該当面の探し方
**オブジェクトプロパティ → RCARDT Node Editor** に面の状態が常時表示され、
**「Select Non-Quad Faces」** ボタンで編集モードに入って該当面だけを選択できます。

四角形化の手順:
1. 「Select Non-Quad Faces」で該当面を選択
2. 隣接する2つの三角形なら `Alt+J`（Tris to Quads）でまとめる
3. 単独の三角形は短い辺を選択して細分化（`Y` → 辺選択 → 細分化）

### 意図的に書き出す場合
`Non-Quad Faces` を **Export Anyway** にすると、三角形はそのまま
（`is_quad = 0`）、N-gonは自動分割して書き出します。検証用の逃げ道であり、
**ゲーム内では該当面が正しく描画されません。**

> 以前の `Auto-Triangulate N-gons`（既定ON）は、N-gonを三角形に変換して
> そのまま出力していました。つまり「描画が壊れる面」を黙って生成する挙動
> だったため、このオプションに置き換えています。
...
| 項目 | 制約 |
|---|---|
| 面の形状 | **四角形のみ。** 三角形を混ぜると描画がバグる（市販版3,341面すべて四角形）。エクスポート時にガードあり |
